...
import { z } from "zod";
import {
  Loader2, Plus, X, Search, Clock, GripVertical,
  CalendarDays, Dumbbell, BookOpen, ChevronDown, StickyNote,
  BarChart3, Filter,
} from "lucide-react";
...
export function SessionForm({ mode, sessionId, availableExercises, initialData, onSuccess, onCancel }: SessionFormProps) {
  ...
  const [dragSrcTimelineIndex, setDragSrcTimelineIndex] = useState<number | null>(null);
  const [isDropZoneActive, setIsDropZoneActive] = useState(false);
  const dragCounter = useRef(0);

  const totalDuration = selectedExercises.reduce((sum, e) => sum + (e.overrideDuration ?? e.durationMinutes), 0);
  ...
  function getCumulativeStart(idx: number) {
    return selectedExercises.slice(0, idx).reduce((sum, e) => sum + (e.overrideDuration ?? e.durationMinutes), 0);
  }
  ...
  function commitDurationEdit(idx: number) {
    ...
    if (!isNaN(val) && val >= 1 && val <= 300) {
      setSelectedExercises(prev => prev.map((e, i) => i === idx ? { ...e, overrideDuration: val } : e));
    }
    setEditingDurationIdx(null);
  }

  function updateNotes(idx: number, notes: string) {
    setSelectedExercises(prev => prev.map((e, i) => i === idx ? { ...e, notes } : e));
  }
  ...
  function addExercise(ex: AvailableExercise, atIndex?: number) {
    const item: SelectedExercise = {
      ...
      durationMinutes: ex.durationMinutes, overrideDuration: null, notes: "",
    };
  ...
  function onDropZoneDragLeave() {
    dragCounter.current--;
    if (dragCounter.current <= 0) { dragCounter.current = 0; setIsDropZoneActive(false); }
  }
  ...
  async function onSubmit(values: FormValues) {
    ...
    if (mode === "create") { router.push("/sessions"); router.refresh(); }
    else { onSuccess?.(); }
  }
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
      ...
      <div className="bg-card border border-border rounded-2xl p-5">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-1.5 sm:col-span-2">
            <label htmlFor="title" className="block text-sm font-semibold text-foreground">Título de la sesión</label>
            <input id="title" type="text" placeholder="Ej: Entrenamiento de técnica ofensiva"
              autoComplete="off" aria-invalid={!!errors.title} {...register("title")}
          ...
          <div className="space-y-1.5">
            <label className="block text-sm font-semibold text-foreground">Fecha y hora</label>
            <Controller name="scheduledAt" control={control} render={({ field }) => (
              <DateTimePicker value={field.value} onChange={field.onChange} error={!!errors.scheduledAt} />
            )} />
          ...
          <div className="space-y-1.5">
            ...
            <textarea id="description" placeholder="Objetivo de la sesión, notas…" rows={3}
              maxLength={2000} {...register("description")}
        ...
        <div className="bg-card border border-border rounded-2xl px-5 py-4 flex flex-col gap-3">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <span className="text-sm font-bold text-foreground">{formatMinutes(totalDuration)}</span>
              <span className="text-xs text-muted-foreground">·</span>
              <span className="text-xs text-muted-foreground">{selectedExercises.length} ejercicio{selectedExercises.length !== 1 ? "s" : ""}</span>
            </div>
          </div>
          {/* Category breakdown bar */}
          <div className="flex h-2 rounded-full overflow-hidden gap-px">
            {breakdown.map(([cat, dur]) => (
              <div key={cat} className="h-full rounded-full transition-all"
                style={{ width: `${(dur / totalDuration) * 100}%`, backgroundColor: CATEGORY_BAR_HEX[cat] ?? "#888" }} />
            ))}
          </div>
          <div className="flex flex-wrap gap-3">
            {breakdown.map(([cat, dur]) => (
              <div key={cat} className="flex items-center gap-1.5 text-xs text-muted-foreground">
                <span className="size-2 rounded-full" style={{ backgroundColor: CATEGORY_BAR_HEX[cat] ?? "#888" }} />
                <span>{CATEGORY_LABELS[cat] ?? cat}</span>
                <span className="font-medium text-foreground">{formatMinutes(dur)}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex flex-col rounded-2xl border border-border overflow-hidden">
          ...
          <div
            className={cn(
              ...
              isDropZoneActive && selectedExercises.length === 0 && "bg-brand/5",
              selectedExercises.length === 0 && "flex items-center justify-center"
            )}
          ...
          >
            {selectedExercises.length === 0 ? (
              <div className={cn(
                ...
                <Dumbbell className={cn("size-8 mb-3", isDropZoneActive ? "text-brand" : "text-muted-foreground")} />
                  ...
                  return (
                    <div key={ex.exerciseId}>
                      ...
                      <div className={cn(
                        ...
                        dragOverTimelineIndex === idx ? "bg-brand" : "bg-transparent"
                      )} />
                      ...
                      <div
                        ...
                        onDragOver={e => { e.preventDefault(); e.stopPropagation(); setDragOverTimelineIndex(idx); }}
                        onDragLeave={() => setDragOverTimelineIndex(null)}
                        onDrop={e => onTimelineItemDrop(e, idx)}
                        className={cn(
                          "group bg-card border rounded-xl transition-all",
                          isDragging ? "opacity-40 border-dashed border-border cursor-grabbing"
                            : "border-border hover:border-brand/30 hover:shadow-sm cursor-grab active:cursor-grabbing"
                        )}
                      >
                        {/* Main row */}
                        <div className="flex items-center gap-2.5 px-3 py-2.5">
                          <div className={cn("w-0.5 h-8 rounded-full shrink-0", CATEGORY_BAR[ex.category] ?? "bg-muted")} />
                          ...
                          <div className="flex flex-col items-center shrink-0 w-10 text-center">
                            <span className="text-[10px] font-mono text-muted-foreground/50">{cumulativeStart}m</span>
                            <span className="text-[10px] font-bold text-muted-foreground/30">▼</span>
                          </div>

                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-foreground truncate leading-snug">{ex.name}</p>
                          ...
                          {isEditingDuration ? (
                            <div className="flex items-center gap-1 shrink-0">
                              <input ref={durationInputRef} type="number" min={1} max={300}
                                value={durationEditValue}
                                onChange={e => setDurationEditValue(e.target.value)}
                                onBlur={() => commitDurationEdit(idx)}
                                onKeyDown={e => {
                                  if (e.key === "Enter") { e.preventDefault(); commitDurationEdit(idx); }
                            ...
                            <button type="button" onClick={() => startEditDuration(idx)} title="Editar duración"
                          ...
                          {/* Notes toggle */}
                          <button type="button"
                            onClick={() => setExpandedNotesIdx(isExpanded ? null : idx)}
                            title="Notas"
                            className={cn(
        ...
        <div className="flex flex-col rounded-2xl border border-border overflow-hidden">
          <div className="flex items-center gap-2 px-4 py-3 border-b border-border bg-muted/30 shrink-0">
            ...
            {filteredAvailable.length > 0 && (
              <span className="ml-auto text-xs text-muted-foreground">{filteredAvailable.length} disponibles</span>
            )}
          </div>
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-border/50 max-h-[400px] lg:max-h-none">
            {filteredAvailable.length === 0 ? (
              <div className="text-center py-8 px-4">
                ...
                <p className="text-xs text-muted-foreground">
                  {search || libCategoryFilter !== "all" ? "Sin resultados para ese filtro" :
                    availableExercises.length === 0 ? "No hay ejercicios en la biblioteca" :
              ...
              filteredAvailable.map(ex => (
                ...
                >
                  <div className={cn("w-0.5 h-7 rounded-full shrink-0", CATEGORY_BAR[ex.category] ?? "bg-muted")} />
                  ...
                  <button type="button" onClick={() => addExercise(ex)}
      ...
      <div className="flex items-center gap-3 pt-1 border-t border-border/60">
        ...
        {onCancel ? (
          ...
          <Link href="/sessions" className="text-sm font-medium text-muted-foreground hover:text-foreground transition-colors px-3 py-2.5">
            Cancelar
          </Link>
        )}
      </div>
