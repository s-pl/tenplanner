...
import {
  ArrowUpRight, Check, Clock, Flame, ChevronRight, Brain,
  Circle, ArrowRight, ArrowLeft, Bot, User, RotateCcw, Trash2, Send, ChevronDown, Sparkles,
} from "lucide-react";
...
function ChatMockup({ withMarginalia = false }: { withMarginalia?: boolean }) {
  return (
    <div className="relative">
      <div className="rounded-[16px] border border-foreground/15 bg-card overflow-hidden shadow-[0_26px_70px_-34px_color-mix(in_oklab,var(--foreground)_42%,transparent)]">
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-foreground/10 bg-card/80 backdrop-blur">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[13px] font-medium text-foreground truncate">Dr. Planner</p>
            <p className="text-[10px] text-foreground/50 truncate">Sesion de volea · Carlos Lopez · 60 min</p>
          </div>
        ...
        <div className="bg-gradient-to-b from-background via-background to-foreground/[0.02] px-4 md:px-5 py-4 md:py-5 space-y-3.5 text-[13px] leading-relaxed">
          ...
          <div className="flex gap-2.5">
            ...
            <div className="max-w-[88%] w-full">
              ...
              <p className="mt-1.5 rounded-xl border border-dashed border-brand/30 bg-brand/[0.04] px-3.5 py-2.5 text-[12px] leading-relaxed text-foreground/72 italic">
                Reviso historial reciente: 3 semanas sin trabajo de volea, nivel intermedio-alto y tolerancia alta a cargas de reaccion.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-start justify-end gap-2.5">
            <div className="max-w-[82%] rounded-2xl rounded-tr-sm bg-brand text-brand-foreground px-4 py-2.5 text-[12px] leading-relaxed">
              Quiero una sesion de 60 min para @Carlos Lopez. Objetivo: volea en situacion de presion.
            </div>
          ...
          <div className="flex gap-2.5">
            ...
            <div className="max-w-[82%] rounded-2xl rounded-tl-sm border border-foreground/10 bg-card px-4 py-2.5 text-[12px] text-foreground/85">
              Perfecto. Te propongo un bloque tecnico progresivo, con subida de intensidad en el tramo central y cierre competitivo.
            </div>
          </div>
          ...
          <div className="ml-9 space-y-2.5">
            <div className="font-sans text-[10px] uppercase tracking-[0.17em] text-foreground/50 flex items-center gap-2">
              <span className="text-brand">▸</span> mostrar_ejercicios <span className="text-foreground/30">· 3 resultados</span>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              {[
                ...
                <div key={e.n} className="rounded-xl border border-foreground/10 bg-card p-2.5">
                  <div className="flex items-center justify-between gap-2">
                    <p className="font-sans text-[9px] uppercase tracking-[0.15em] text-foreground/45">{e.c}</p>
                  ...
                  <p className="mt-1 text-[11px] font-semibold leading-tight text-foreground">{e.n}</p>
                  <div className="mt-2 flex items-center justify-between">
                    <span className="font-sans text-[9px] text-foreground/50 tabular-nums">{e.d}</span>
          ...
          <div className="ml-9 rounded-xl border border-foreground/10 bg-card px-3.5 py-3">
            <div className="grid sm:grid-cols-2 gap-x-5 gap-y-1.5 text-[11px]">
              <span className="font-sans text-[9px] uppercase tracking-[0.14em] text-foreground/45">Objetivo</span>
              <span className="text-foreground/85">Volea bajo presion</span>
              <span className="font-sans text-[9px] uppercase tracking-[0.14em] text-foreground/45">Intensidad</span>
              <span className="flex gap-0.5">
                {[1, 2, 3, 4, 5].map((n) => (
                  <span key={n} className={`h-1 w-4 rounded-full ${n <= 4 ? "bg-brand" : "bg-foreground/15"}`} />
                ))}
              </span>
              <span className="font-sans text-[9px] uppercase tracking-[0.14em] text-foreground/45">Fecha</span>
              <span className="text-foreground/85 tabular-nums">Mie 23 abr · 18:00</span>
            </div>
        ...
        <div className="border-t border-foreground/10 bg-card/80 px-4 py-3.5">
          <div className="flex items-center gap-2 rounded-2xl border border-foreground/12 bg-background px-3 py-2">
            <span className="flex-1 truncate text-[12px] leading-none text-foreground/42">Describe la sesion que necesitas...</span>
      ...
      {withMarginalia && (
        <>
          <Marginalia y={150} tone="brand">razonamiento expuesto</Marginalia>
...
const DR_TOOLS = [
  ...
  { id: "02", name: "analizar_historial",   desc: "Detecta bloqueos y progresión" },
    ...
    <div className="rounded-[14px] border border-foreground/15 bg-card overflow-hidden shadow-[0_24px_60px_-30px_color-mix(in_oklab,var(--foreground)_40%,transparent)]">
      ...
      <div className="flex items-center gap-3 px-5 py-3 border-b border-foreground/10 bg-foreground/[0.02]">
        ...
        <div className="flex-1">
          <p className="font-sans text-[9px] uppercase tracking-[0.22em] text-foreground/45">Dr. Planner</p>
          <p className="font-heading italic text-[15px] leading-none text-foreground">Caja de herramientas</p>
        </div>
        <span className="font-sans text-[10px] tracking-[0.18em] text-brand">07 · TOOLS</span>
      </div>
      ...
      <ul className="divide-y divide-foreground/8">
        {DR_TOOLS.map((t) => (
          ...
          >
            <span className="font-sans text-[10px] tabular-nums text-foreground/35">{t.id}</span>
            <span className="text-brand font-sans text-[11px]">▸</span>
            <div>
              <p className="font-sans text-[12px] text-foreground/90">{t.name}</p>
      ...
      <div className="border-t border-foreground/10 bg-foreground/[0.015] px-5 py-4">
        <p className="font-sans text-[9px] uppercase tracking-[0.2em] text-foreground/45 mb-3">Ciclo de razonamiento</p>
        <div className="flex items-center gap-1.5 text-[11px]">
          {["Lee", "Piensa", "Propone", "Confirma"].map((s, i) => (
            <div key={s} className="flex items-center gap-1.5">
              ...
              {i < 3 && <ArrowRight className="size-3 text-foreground/30" strokeWidth={1.5} />}
            </div>
          ))}
        </div>
      </div>
...
const LIBRARY_EXERCISES = [
  { name: "Volea de derecha — pared", cat: "TÉC", diff: "INT", dur: "12", tag: "01" },
  { name: "Cruzado y bajada de pared", cat: "TÉC", diff: "AVZ", dur: "10", tag: "02" },
  { name: "Remate desde pared", cat: "TÉC", diff: "AVZ", dur: "12", tag: "03" },
  { name: "2v2 red con presión", cat: "TÁC", diff: "AVZ", dur: "15", tag: "04" },
  { name: "Construcción al fondo", cat: "TÁC", diff: "INT", dur: "10", tag: "05" },
  { name: "Circuito de agilidad", cat: "FÍS", diff: "INT", dur: "10", tag: "06" },
  { name: "Core y estabilidad", cat: "FÍS", diff: "PRN", dur: "12", tag: "07" },
  { name: "Calentamiento dinámico", cat: "CAL", diff: "·", dur: "08", tag: "08" },
  ...
  return (
    <div className="rounded-xl border border-foreground/15 bg-card overflow-hidden">
      <div className="flex items-center gap-3 px-4 py-3 border-b border-foreground/10">
        <span className="font-sans text-[10px] tracking-[0.2em] text-foreground/55">BIBLIOTECA</span>
        <span className="font-sans text-[10px] text-foreground/35 tabular-nums">/ 137 EJERCICIOS</span>
        <span className="ml-auto font-sans text-[10px] tracking-[0.15em] text-brand">+ AI</span>
      </div>
      <ul className="divide-y divide-foreground/8">
        {LIBRARY_EXERCISES.map((e) => (
          ...
          >
            <span className="font-sans text-[10px] tabular-nums text-foreground/35 w-6">{e.tag}</span>
            <span
              className={
            ...
            <span className="text-[13px] text-foreground/85 truncate">{e.name}</span>
            <span className="font-sans text-[10px] tracking-[0.1em] text-foreground/40">{e.diff}</span>
  ...
  return (
    <div className="rounded-xl border border-foreground/15 bg-card p-5">
      <div className="flex items-baseline justify-between mb-4">
        <div>
          <p className="font-sans text-[10px] tracking-[0.2em] text-foreground/55">CURVA DE INTENSIDAD</p>
          <p className="font-heading italic text-xl leading-tight mt-0.5">Técnica de volea · 60&apos;</p>
        </div>
      ...
      <div className="relative">
        <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto overflow-visible">
          ...
          {/* phase bands */}
          <rect x={mapX(0)} y={20} width={mapX(10) - mapX(0)} height={H - 50} fill="currentColor" opacity="0.025" />
          <rect x={mapX(50)} y={20} width={mapX(60) - mapX(50)} height={H - 50} fill="currentColor" opacity="0.025" />
          {/* area */}
          <path
          ...
          {points.map((p, i) => (
            <g key={i}>
              <circle cx={mapX(p.x)} cy={mapY(p.y)} r={4} fill="var(--background)" stroke="var(--brand)" strokeWidth="2" />
            </g>
          ))}
          {/* phase labels */}
          <g fontFamily="var(--font-sans)" fontSize="9" fill="currentColor" opacity="0.45" letterSpacing="0.5">
            <text x={mapX(4)} y={H - 8} textAnchor="middle">CALENT.</text>
            <text x={mapX(30)} y={H - 8} textAnchor="middle">PRINCIPAL</text>
            <text x={mapX(55)} y={H - 8} textAnchor="middle">V. CALMA</text>
          </g>
          {/* minute marks */}
          <g fontFamily="var(--font-sans)" fontSize="8" fill="currentColor" opacity="0.35">
          ...
          <line x1={mapX(10)} y1={20} x2={mapX(10)} y2={H - 30} stroke="currentColor" strokeWidth="0.6" opacity="0.15" strokeDasharray="2 3" />
          <line x1={mapX(50)} y1={20} x2={mapX(50)} y2={H - 30} stroke="currentColor" strokeWidth="0.6" opacity="0.15" strokeDasharray="2 3" />
        </svg>
      </div>

      <div className="mt-4 grid grid-cols-3 gap-4 pt-4 border-t border-foreground/10">
        <div>
          <p className="font-sans text-[9px] tracking-[0.18em] text-foreground/40">DURACIÓN</p>
          <p className="font-heading text-xl mt-0.5 tabular-nums">60<span className="text-foreground/40">&apos;</span></p>
        </div>
        <div>
          <p className="font-sans text-[9px] tracking-[0.18em] text-foreground/40">EJERCICIOS</p>
          <p className="font-heading text-xl mt-0.5 tabular-nums">07</p>
        </div>
        <div>
          <p className="font-sans text-[9px] tracking-[0.18em] text-foreground/40">CARGA</p>
...
const STUDENTS = [
  { n: "Carlos López",  age: 34, lvl: "INTERMEDIO",  hand: "D", grip: "Cont.",  exp: 4, sess: 18, trend: "+" },
  { n: "María Torres",  age: 28, lvl: "AVANZADO",    hand: "D", grip: "SemiO.", exp: 7, sess: 42, trend: "+" },
  { n: "Pedro García",  age: 41, lvl: "AMATEUR",     hand: "Z", grip: "Cont.",  exp: 2, sess: 9,  trend: "·" },
  { n: "Ana Ruiz",      age: 22, lvl: "COMPETICIÓN", hand: "D", grip: "Occ.",   exp: 9, sess: 68, trend: "+" },
];
  ...
  return (
    <div className="rounded-xl border border-foreground/15 bg-card overflow-hidden">
      <div className="px-5 py-3 border-b border-foreground/10 flex items-baseline justify-between">
        <p className="font-sans text-[10px] tracking-[0.2em] text-foreground/55">PLANTILLA · 04</p>
        <p className="font-sans text-[10px] tracking-[0.15em] text-foreground/35">137 SESIONES TOTALES</p>
      </div>
        ...
        {STUDENTS.map((s, i) => (
          ...
          >
            ...
            <span className="font-sans text-[12px] font-bold tabular-nums text-foreground text-right">
              ...
              <span className={s.trend === "+" ? "text-brand" : "text-foreground/30"}>{s.trend}</span>
            </span>
          </li>
        ))}
      </ul>
...
function CalendarMonth() {
  ...
  const sessions: Record<number, { t: string; phase: "tec" | "fis" | "tac" | "cal" }[]> = {
    1:  [{ t: "Carlos", phase: "tec" }],
    2:  [{ t: "María",  phase: "tac" }],
    6:  [{ t: "Ana",    phase: "fis" }, { t: "Pedro", phase: "cal" }],
    8:  [{ t: "Carlos", phase: "tec" }],
    9:  [{ t: "Grupo A", phase: "tac" }],
    13: [{ t: "Ana",    phase: "fis" }],
    15: [{ t: "Carlos", phase: "tec" }],
    16: [{ t: "María",  phase: "tac" }, { t: "Ana", phase: "tec" }],
  ...
  return (
    <div className="rounded-xl border border-foreground/15 bg-card overflow-hidden">
      <div className="flex items-baseline justify-between px-5 py-3 border-b border-foreground/10">
        <div>
          <p className="font-sans text-[10px] tracking-[0.2em] text-foreground/55">CALENDARIO</p>
          <p className="font-heading text-[22px] leading-none mt-1">abril <span className="italic text-foreground/55">26</span></p>
        </div>
        <div className="text-right">
          <p className="font-sans text-[10px] tracking-[0.15em] text-foreground/40">SESIONES</p>
          <p className="font-heading text-xl tabular-nums">14</p>
        </div>
      </div>
      <div className="grid grid-cols-7 border-b border-foreground/10">
        {days.map((d) => (
          <div key={d} className="text-center font-sans text-[9px] tracking-[0.18em] text-foreground/35 py-2 border-r border-foreground/5 last:border-0">
            {d}
          </div>
        ))}
      </div>
...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-background text-foreground selection:bg-brand/25">
        ...
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10">
          <div className="flex items-end justify-between py-4">
            <Link href="/" className="flex items-baseline gap-2">
              ...
              <span className="font-sans text-[10px] tracking-[0.18em] text-foreground/40 pb-1">™</span>
            </Link>
            <nav className="hidden md:flex items-end gap-8 text-sm">
              {[
                ...
                <a key={l} href={h} className="text-foreground/60 hover:text-foreground transition-colors">
                  {l}
                </a>
              ))}
            </nav>
            <div className="flex items-center gap-4">
              <Link href="/login" className="hidden sm:inline text-sm text-foreground/55 hover:text-foreground transition-colors">
                Entrar
              </Link>
              <Link
                href="/register"
              ...
              >
                ...
                <span className="inline-flex items-center justify-center size-6 rounded-full bg-background/15">
                  <ArrowUpRight className="size-3.5 group-hover:rotate-45 transition-transform" strokeWidth={2} />
                </span>
              </Link>
            </div>
          </div>
      ...
      <section className="relative overflow-hidden border-b border-foreground/10">
        ...
        <div className="relative max-w-[1400px] mx-auto px-6 lg:px-10 pt-10 lg:pt-14 pb-16 lg:pb-20">
          <div className="grid lg:grid-cols-[1.2fr_1fr] gap-12 lg:gap-20 items-start">
            ...
            {/* Headline */}
            <div>
              <h1 className="mt-1 font-heading font-normal text-foreground leading-[0.88] tracking-[-0.02em]"
                  style={{ fontSize: "clamp(3.2rem, 9vw, 8.25rem)" }}>
                Diseña la sesión<br />
                <span className="italic text-brand">mientras piensas</span><br />
                en el próximo<br />
                partido.
              </h1>
        ...
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10 py-6 grid grid-cols-2 md:grid-cols-4 gap-y-6 gap-x-8">
          {[
            ...
            <div key={s.k} className="flex flex-col">
              ...
              <span className="font-heading text-2xl lg:text-[30px] leading-tight mt-1">{s.v}</span>
              <span className="text-[11px] text-foreground/55 mt-0.5">{s.sub}</span>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10 py-20 lg:py-28">
          <div className="mt-2 grid lg:grid-cols-[1fr_1.1fr] gap-12 lg:gap-20 items-start">
            <div className="lg:sticky lg:top-40">
              <h2 className="font-heading font-normal leading-[0.92] tracking-[-0.015em]"
                  style={{ fontSize: "clamp(2.4rem, 5.2vw, 4.5rem)" }}>
                Un asistente que <span className="italic text-brand">piensa</span> antes de proponer.
              </h2>
              ...
              <dl className="mt-10 space-y-0 border-t border-foreground/15">
                {[
                  ["Lee historial", "Detecta bloqueos y progresión real del alumno."],
                  ["Razonamiento visible", "Ves el por qué de cada decisión, no solo el qué."],
                  ["Crea si falta", "Si no encuentra el ejercicio adecuado, lo genera."],
                  ["Nunca se sobrepasa", "Pide confirmación antes de cualquier cambio."],
                ].map(([k, v]) => (
                  <div key={k} className="grid grid-cols-[130px_1fr] gap-6 border-b border-foreground/15 py-4">
                    <dt className="font-sans text-[11px] uppercase tracking-[0.15em] text-foreground/55">{k}</dt>
                    <dd className="text-[14px] text-foreground/80 leading-relaxed">{v}</dd>
                  </div>
                ))}
              </dl>
            </div>
      ...
      {/* ── Biblioteca ───────────────────────────────────────── */}
      <section id="biblioteca" className="relative border-b border-foreground/10 bg-foreground/[0.015]">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10 py-20 lg:py-28">
          <div className="mt-2 grid lg:grid-cols-[1.1fr_1fr] gap-12 lg:gap-20 items-start">
            ...
            <div className="lg:pl-4">
              <h2 className="font-heading font-normal leading-[0.92] tracking-[-0.015em]"
                ...
                Cada <span className="italic">drill</span> documentado.<br />
                Ninguno repetido.
              </h2>
              ...
              <div className="mt-10 grid grid-cols-2 gap-x-8 gap-y-6">
                {[
                  { tag: "TÉC", n: 48, d: "Técnica" },
                  { tag: "TÁC", n: 39, d: "Táctica" },
                  { tag: "FÍS", n: 31, d: "Físico" },
                  { tag: "CAL", n: 19, d: "Calentamiento" },
                ].map((c) => (
                  <div key={c.tag} className="border-t border-foreground/20 pt-3">
                    <div className="flex items-baseline justify-between">
                      <span className="font-sans text-[10px] tracking-[0.2em] text-foreground/50">{c.tag}</span>
                      <span className="font-heading text-[32px] tabular-nums">{c.n}</span>
                    </div>
                    <p className="text-[12px] text-foreground/60 mt-0.5">{c.d}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="mt-2 grid lg:grid-cols-12 gap-8 lg:gap-12">
            <div className="lg:col-span-4">
              <h2 className="font-heading font-normal leading-[0.92] tracking-[-0.015em]"
                ...
                De la <span className="italic text-brand">activación</span><br />
                a la vuelta<br />
                a la calma.
              </h2>
              ...
              <div className="mt-8 space-y-0">
                {[
                  { p: "ACTIVACIÓN", t: "10'", ex: ["Movilidad articular", "Calentamiento dinámico"] },
                  { p: "PRINCIPAL", t: "35'", ex: ["Volea de derecha — pared", "Cruzado y bajada", "Táctica 2v2 red"] },
                  { p: "V. CALMA", t: "15'", ex: ["Circuito de agilidad", "Estiramientos"] },
                ].map((ph) => (
                  <div key={ph.p} className="border-t border-foreground/15 py-4">
                    <div className="flex items-baseline justify-between mb-2">
                      <span className="font-sans text-[10px] tracking-[0.2em] text-foreground/55">{ph.p}</span>
                      <span className="font-sans text-[11px] tabular-nums text-foreground/40">{ph.t}</span>
                    </div>
                    <ul className="space-y-1">
                      {ph.ex.map((e) => (
                        <li key={e} className="flex items-baseline gap-2 text-[13px] text-foreground/80">
                ...
                <div className="border-t border-foreground/15 pt-4 flex items-baseline justify-between">
                  <span className="font-sans text-[10px] tracking-[0.2em] text-foreground">TOTAL</span>
                  <span className="font-heading text-2xl tabular-nums">60&apos;</span>
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-8 space-y-6">
              ...
              <div className="grid sm:grid-cols-3 gap-4">
                {[
                  { icon: <Flame className="size-4" />, k: "Carga balanceada", v: "Pico en minuto 40" },
                  { icon: <Clock className="size-4" />, k: "Ritmo", v: "58\" por ejercicio en promedio" },
                  { icon: <Check className="size-4" />, k: "Coherencia", v: "Cuadra con duración pedida" },
                ].map((s) => (
                  <div key={s.k} className="rounded-xl border border-foreground/15 bg-card p-4">
                    <div className="flex items-center gap-2 text-brand">{s.icon}<span className="font-sans text-[10px] tracking-[0.18em] uppercase">Análisis</span></div>
                    <p className="mt-2 text-[13px] font-medium">{s.k}</p>
                    <p className="text-[12px] text-foreground/60 mt-0.5">{s.v}</p>
                  </div>
                ))}
              </div>
            </div>
      ...
      {/* ── Alumnos ──────────────────────────────────────────── */}
      <section id="alumnos" className="relative border-b border-foreground/10 bg-foreground/[0.015]">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10 py-20 lg:py-28">
          <div className="mt-2 grid lg:grid-cols-[1fr_1.3fr] gap-12 lg:gap-20 items-start">
            <div className="lg:sticky lg:top-40">
              <h2 className="font-heading font-normal leading-[0.92] tracking-[-0.015em]"
                  style={{ fontSize: "clamp(2.2rem, 4.8vw, 4rem)" }}>
                Conoce a tu <span className="italic">plantilla</span>.<br />
                <span className="text-foreground/55">El asistente también.</span>
              </h2>
              ...
              <blockquote className="mt-8 font-heading italic text-[22px] leading-snug text-foreground/80 pl-4 border-l-2 border-brand">
                &ldquo;Carlos lleva tres sesiones sin volea. Le preparo un bloque principal de técnica con presión progresiva.&rdquo;
      ...
      {/* ── Calendario ───────────────────────────────────────── */}
      <section id="calendario" className="relative border-b border-foreground/10">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10 py-20 lg:py-28">
          <div className="mt-2 grid lg:grid-cols-[1.3fr_1fr] gap-12 lg:gap-20 items-start">
            ...
            <div className="lg:pl-4">
              ...
              <p className="mt-6 text-[15px] leading-relaxed text-foreground/70 max-w-md">
                Una semana o un mes completo, sesión a sesión. Etiqueta por alumno, fase dominante o tipo de trabajo. Cambia el plan sin perder el hilo del ciclo.
              </p>

              <div className="mt-10 space-y-3 font-sans text-[10px] tracking-[0.18em] uppercase text-foreground/55">
                <div className="flex items-center gap-2.5"><span className="inline-block h-2 w-4 rounded-sm bg-brand" /> TÉCNICA</div>
                <div className="flex items-center gap-2.5"><span className="inline-block h-2 w-4 rounded-sm border border-brand/40 bg-brand/[0.08]" /> TÁCTICA</div>
                <div className="flex items-center gap-2.5"><span className="inline-block h-2 w-4 rounded-sm border border-foreground/25" /> FÍSICO</div>
                <div className="flex items-center gap-2.5"><span className="inline-block h-2 w-4 rounded-sm border border-dashed border-foreground/20" /> CALENTAMIENTO</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="relative border-b border-foreground/10">
        ...
        <div className="relative max-w-[1400px] mx-auto px-6 lg:px-10 py-24 lg:py-36 text-center">
          <h2 className="mt-2 font-heading font-normal leading-[0.9] tracking-[-0.02em] max-w-4xl mx-auto"
              style={{ fontSize: "clamp(2.6rem, 7vw, 6rem)" }}>
            Un entrenador con <span className="italic text-brand">método</span><br />
            gana más que un entrenador<br />
            con <span className="italic">talento</span>.
          </h2>
          ...
          <div className="mt-12 flex items-center justify-center gap-4">
            ...
            >
              ...
              <span className="inline-flex items-center justify-center size-8 rounded-full bg-background/15">
                <ArrowUpRight className="size-4 group-hover:rotate-45 transition-transform" strokeWidth={2} />
              </span>
            </Link>
            <Link href="/login" className="text-[14px] text-foreground/60 hover:text-foreground underline underline-offset-4 decoration-foreground/25 transition-colors">
              Ya tengo cuenta
            </Link>
          </div>
      ...
      <footer className="bg-background">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10 py-10">
              ...
              <div key={col.t}>
                ...
                <ul className="space-y-2.5">
                  {col.l.map(([label, href]) => (
                    <li key={label}>
                      <Link href={href} className="text-[13px] text-foreground/80 hover:text-brand transition-colors">
                        {label}
                      </Link>
                    </li>
                  ))}
          ...
          <div className="pt-6 text-[12px] leading-relaxed text-foreground/55">
            <span>© {new Date().getFullYear()} TenPlanner · Todos los derechos reservados</span>
          </div>
        </div>
      </footer>
    </div>
