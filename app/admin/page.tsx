...
export default async function AdminDashboard() {
  ...
  const maxSignups = Math.max(
    ...metrics.signupsByDay.map((r) => Number(r.total)),
    1
  );
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-8 p-4 sm:p-6 lg:p-8">
      ...
      <section>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 lg:gap-4">
          {contentStats.map((s) => (
            <div
              key={s.label}
              className="rounded-2xl border border-foreground/10 bg-foreground/[0.02] p-4 sm:p-5"
              ...
              {metrics.signupsByDay
                ...
                .map((row) => (
                  <div key={row.day} className="flex items-center gap-3">
                    <span className="w-16 shrink-0 font-mono text-[10px] text-foreground/40 sm:w-24 sm:text-[11px]">
            ...
            <div className="space-y-3">
              <div>
                <div className="flex justify-between text-xs text-foreground/50 mb-1">
                  <span>Globales</span>
                  <span>
                    {metrics.globalExercises} (
                    {Math.round(
                      (metrics.globalExercises / metrics.totalExercises) * 100
                    )}
                    %)
                  </span>
                </div>
                <div className="h-2 rounded-full bg-foreground/8">
                  <div
                    ...
                    style={{
                      width: `${(metrics.globalExercises / metrics.totalExercises) * 100}%`,
                    }}
                  />
                </div>
              </div>
              <div>
                <div className="flex justify-between text-xs text-foreground/50 mb-1">
                  ...
                  <span>
                    {metrics.aiExercises} (
                    {Math.round(
                      (metrics.aiExercises / metrics.totalExercises) * 100
                    )}
                    %)
                  </span>
                </div>
                <div className="h-2 rounded-full bg-foreground/8">
                  <div
                    ...
                    style={{
                      width: `${(metrics.aiExercises / metrics.totalExercises) * 100}%`,
                    }}
                  />
                </div>
              </div>
