...
export async function POST(request: Request) {
  ...
  const [
    ...
    db
      ...
      .from(exercises)
      .where(eq(exercises.createdBy, user.id)),
    db
      .select(EXERCISE_FIELDS)
      .from(exercises)
...
${globalExercises.map(formatExerciseLine).join("\n")}`;
  ...
  const result = streamText({
    ...
    tools: {
        ...
        execute: async ({
          ...
          if (!conversationState.hasExerciseCreationConsent) {
            return {
              ok: false,
              error:
                "Falta permiso explícito para crear ejercicios nuevos. Pide confirmación antes de crear.",
            };
          }
        ...
        execute: async ({
          ...
          try {
            ...
            const durMap = new Map(
              exerciseData.map((e) => [e.id, e.durationMinutes])
            );
      ...
      analizar_alumno: {
        description:
          "Muestra perfil completo de un alumno con su historial de sesiones y estadísticas. Usar cuando el entrenador pregunte por un alumno o quiera adaptar una sesión a su historial.",
        inputSchema: z.object({
        ...
        execute: async ({ studentId }) => {
          const [student] = await db
            ...
            .from(students)
            .where(
              and(eq(students.id, studentId), eq(students.coachId, user.id))
            )
            .limit(1);
