    ...
    <div className="relative">
      ...
      <div className="relative px-4 sm:px-6 md:px-10 py-10 space-y-10">
          ...
          <div className="grid grid-cols-[auto_1fr] gap-6 items-end">
            <div className="size-20 rounded-full border border-foreground/25 bg-foreground/[0.02] flex items-center justify-center shrink-0 overflow-hidden">
              {student.imageUrl ? (
                ...
                <span className="font-heading text-2xl text-foreground/80">{initialsFromName(student.name)}</span>
              )}
            </div>
            <div className="min-w-0">
              ...
              <div className="flex flex-wrap items-center gap-x-5 gap-y-1 mt-3 text-[12px] tabular-nums">
                ...
                {student.email && (
                  <span className="text-foreground/50 truncate">{student.email}</span>
                )}
              </div>
            </div>
          </div>
        ...
        <section className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 border-y border-foreground/15">
          ...
          <StatCell label="Horas totales" value={`${Math.round(totalMinutes / 60)}h`} />
          <StatCell label="Intensidad media" value={avgIntensity != null ? `${avgIntensity.toFixed(1)}/5` : "—"} />
          <StatCell label="Valoración media" value={avgRating != null ? `${avgRating.toFixed(1)}/5` : "—"} />
        </section>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-[3fr_2fr] gap-10">
          <div>
            ...
            <dl className="grid grid-cols-2 md:grid-cols-3 divide-x divide-y divide-foreground/10 border-b border-foreground/10">
              <InfoRow label="Género" value={student.gender ? GENDER_LABEL[student.gender as Gender] : "—"} />
              ...
              <InfoRow label="Mano" value={student.dominantHand ? HAND_LABEL[student.dominantHand as DominantHand] : "—"} />
              <InfoRow label="Altura" value={student.heightCm ? `${student.heightCm} cm` : "—"} />
              <InfoRow label="Peso" value={student.weightKg ? `${student.weightKg} kg` : "—"} />
              <InfoRow label="Experiencia" value={student.yearsExperience != null ? `${student.yearsExperience} año${student.yearsExperience !== 1 ? "s" : ""}` : "—"} />
            </dl>
            {student.notes && (
              <div className="mt-6">
          ...
          <aside>
            <div className="grid grid-cols-[auto_1fr] items-baseline gap-3 pb-3 border-b border-foreground/15">
              <p className="font-sans text-[10px] tabular-nums tracking-[0.22em] text-brand">02</p>
              <p className="font-sans text-[10px] uppercase tracking-[0.22em] text-foreground/50">
                Enlace de perfil
              </p>
            </div>
            <p className="text-[13px] text-foreground/60 leading-relaxed mt-4 mb-4">
              Genera un enlace temporal para que el alumno rellene sus datos físicos sin necesidad de cuenta. Caduca en 7 días.
            </p>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          <div>
            <div className="grid grid-cols-[auto_1fr] items-baseline gap-3 pb-3 border-b border-foreground/15">
              <p className="font-sans text-[10px] tabular-nums tracking-[0.22em] text-brand">03</p>
              <p className="font-sans text-[10px] uppercase tracking-[0.22em] text-foreground/50">
            ...
            {pastWithAttendanceCount === 0 ? (
              <p className="text-[13px] text-foreground/55 mt-4">
                Registra la asistencia en sesiones pasadas para ver esta métrica.
              </p>
            ) : (
                ...
                <div className="h-px bg-foreground/10 relative overflow-hidden">
                  <div
                    className={cn(
                      ...
                      (attendanceRate ?? 0) >= 0.8 ? "bg-brand" : (attendanceRate ?? 0) >= 0.5 ? "bg-foreground/60" : "bg-destructive"
                    )}
          ...
          <div>
            <div className="grid grid-cols-[auto_1fr] items-baseline gap-3 pb-3 border-b border-foreground/15">
              <p className="font-sans text-[10px] tabular-nums tracking-[0.22em] text-brand">04</p>
              <p className="font-sans text-[10px] uppercase tracking-[0.22em] text-foreground/50">
                Proyección · ritmo
              </p>
            </div>
            <div className="grid grid-cols-3 gap-4 mt-5">
              <div>
                ...
                {nextSession ? (
                  <Link href={`/sessions/${nextSession.sessionId}`} className="group block">
              ...
              <div>
                ...
                <p className="text-[11px] text-foreground/50 mt-1">ses/semana · 30d</p>
              </div>
              <div>
                <p className="font-sans text-[9px] uppercase tracking-[0.22em] text-foreground/45 mb-1">
                  Acumuladas
                ...
                <p className="text-[11px] text-foreground/50 mt-1">entrenadas</p>
              </div>
            </div>
          </div>
        </section>
        ...
        {recentEvolution.length > 0 && (
          <section>
            <div className="grid grid-cols-[auto_1fr_auto] items-baseline gap-3 pb-3 border-b border-foreground/15">
              <p className="font-sans text-[10px] tabular-nums tracking-[0.22em] text-brand">05</p>
              <p className="font-sans text-[10px] uppercase tracking-[0.22em] text-foreground/50">
                Evolución reciente
              </p>
            ...
            <div className="mt-5 grid grid-cols-1 md:grid-cols-2 gap-8">
              <EvolutionChart label="Intensidad" items={recentEvolution.map((s) => s.intensity)} />
              <EvolutionChart label="Valoración" items={recentEvolution.map((s) => s.rating)} />
            </div>
          </section>
        )}
        ...
        <section>
          <div className="grid grid-cols-[auto_1fr_auto] items-baseline gap-3 pb-3 border-b border-foreground/15">
            <p className="font-sans text-[10px] tabular-nums tracking-[0.22em] text-brand">06</p>
            <p className="font-sans text-[10px] uppercase tracking-[0.22em] text-foreground/50">
              Próximas sesiones
            </p>
            <p className="font-sans text-[10px] tabular-nums tracking-[0.22em] text-foreground/40">
        ...
        <section>
          <div className="grid grid-cols-[auto_1fr_auto] items-baseline gap-3 pb-3 border-b border-foreground/15">
            <p className="font-sans text-[10px] tabular-nums tracking-[0.22em] text-brand">07</p>
            <p className="font-sans text-[10px] uppercase tracking-[0.22em] text-foreground/50">
          ...
          {past.length === 0 ? (
            ...
            <ul className="divide-y divide-foreground/10">
              {past.map((s, idx) => (
                <li key={s.ssId} className="py-5 grid grid-cols-1 lg:grid-cols-[auto_1fr_1fr] gap-5 items-start">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="size-9 border border-foreground/20 flex items-center justify-center shrink-0">
                      <ClipboardList className="size-3.5 text-foreground/60" strokeWidth={1.6} />
                    </div>
                  </div>
                  <div className="min-w-0">
                    <Link
                    ...
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-0.5 mt-1 text-[11px] text-foreground/55 tabular-nums">
                      ...
                      <span className="inline-flex items-center gap-1">
                        <Clock className="size-3" strokeWidth={1.6} /> {s.durationMinutes}′
                      </span>
                      {s.intensity != null && (
                        <span className="inline-flex items-center gap-1">
                          <Flame className="size-3" strokeWidth={1.6} /> {s.intensity}/5
                        </span>
                      )}
                    </div>
                    {s.objective && (
...
function EvolutionChart({ label, items }: { label: string; items: Array<number | null> }) {
  const max = 5;
  return (
    <div>
      ...
      <div className="flex items-start gap-1.5 mt-1.5">
        {items.map((v, i) => (
          <span key={i} className="flex-1 text-center font-sans text-[9px] tabular-nums text-foreground/40">
            {v ?? "—"}
          </span>
        ))}
      </div>
