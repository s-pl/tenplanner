    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => !deleting && setOpen(false)} />
          <div className="relative bg-card border border-border rounded-2xl p-6 w-full max-w-sm shadow-2xl">
            <div className="flex items-start gap-4 mb-5">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="font-semibold text-foreground">Eliminar alumno</h3>
                <p className="text-sm text-muted-foreground mt-1 leading-relaxed">
                  ¿Seguro que quieres eliminar a <span className="font-medium text-foreground">{studentName}</span>? Esta acción no se puede deshacer.
                </p>
              </div>
              <button onClick={() => !deleting && setOpen(false)}
            ...
            <div className="flex gap-3">
              <button onClick={() => !deleting && setOpen(false)} disabled={deleting}
