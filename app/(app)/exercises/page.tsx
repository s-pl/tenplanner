...
export default async function ExercisesPage({ searchParams }: PageProps) {
  ...
  const activeCategory = (
    CATEGORIES.includes(category as never) ? category : "all"
  ) as "all" | Category;
  const activeDifficulty = (
    DIFFICULTIES.includes(difficulty as never) ? difficulty : "all"
  ) as "all" | Difficulty;
  ...
  function buildHref(params: Record<string, string | undefined>) {
    ...
    if (params.difficulty && params.difficulty !== "all")
      p.set("difficulty", params.difficulty);
          ...
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 pb-6 border-b border-foreground/15">
            <div className="max-w-2xl">
              ...
              <p className="mt-3 text-[15px] text-foreground/65 leading-relaxed">
                Una colección viva: ejercicios globales, propios y generados con
                IA. El material del que nacen todas las sesiones.
              </p>
            </div>
            <Link
              href="/exercises/new"
        ...
        <div className="flex flex-col md:flex-row gap-3">
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto">
            ...
            {DIFFICULTIES.map((diff) => {
              ...
              const label =
                diff === "all" ? "Todos" : DIFFICULTY_LABEL[diff as Difficulty];
              return (
                <Link
                  key={diff}
                  href={buildHref({
