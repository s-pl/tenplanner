...
import {
  ArrowLeft, Pencil, Trash2, X, Loader2, Clock, Target, Brain,
  Dumbbell, Flame, MapPin, Video, Lightbulb, Package, ListOrdered,
  ExternalLink,
} from "lucide-react";
...
const CATEGORY_META: Record<Category, { label: string; color: string; bg: string; icon: React.ElementType }> = {
    ...
    return (
      <div className="px-4 md:px-8 py-8 space-y-6 max-w-5xl">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="font-heading text-3xl font-bold tracking-tight text-foreground">Editar ejercicio</h1>
    ...
    <div className="px-4 md:px-8 py-8 space-y-6 max-w-5xl">
      ...
      <div className="bg-card border border-border rounded-2xl overflow-hidden">
        ...
        <div className="p-6 space-y-6">
          <h1 className="font-heading text-2xl font-bold tracking-tight text-foreground leading-snug">{exercise.name}</h1>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {exercise.materials.map(m => (
                  <span key={m} className="text-xs font-medium bg-muted text-foreground px-2.5 py-1 rounded-lg">{m}</span>
                ))}
              </div>
            </div>
          )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-card border border-border rounded-2xl p-6 w-full max-w-sm shadow-2xl">
            <div className="flex items-start gap-4 mb-5">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-muted-foreground mt-1 leading-relaxed">
                  ¿Seguro que quieres eliminar <span className="font-medium text-foreground">{exercise.name}</span>? Esta acción no se puede deshacer.
                </p>
              </div>
              <button onClick={() => !deleting && setShowDeleteConfirm(false)}
            ...
            <div className="flex gap-3">
              <button onClick={() => !deleting && setShowDeleteConfirm(false)} disabled={deleting}
